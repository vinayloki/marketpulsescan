"""
MarketPulse — 52-Week High Breakout Scanner
"""
...
class BreakoutScanner(BaseScanner):
    ...
    def scan(self, ohlcv: pd.DataFrame) -> dict[str, ScanResult]:
        ...
        if "Close" not in ohlcv.columns.get_level_values(0):
            log.warning("BreakoutScanner: No 'Close' column in OHLCV")
            return results
        if "High" not in ohlcv.columns.get_level_values(0):
            log.warning("BreakoutScanner: No 'High' column in OHLCV")
            return results
        if "Volume" not in ohlcv.columns.get_level_values(0):
            log.warning("BreakoutScanner: No 'Volume' column in OHLCV")
            return results
        ...
        for ticker in tickers:
            try:
                ...
                pct_from_high = (1.0 - proximity_ratio) * 100
                ...
                # Volume quality bonus (0-10 pts, linear up to 3x)
                ...
                log.debug("BreakoutScanner skip %s: %s", ticker, e)
                continue

        log.info("BreakoutScanner: %d signals found", triggered_count)
        return results
